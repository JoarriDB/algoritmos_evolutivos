...
def calcular_fitness(cromosoma):
    ...
    # Cálculo de promedios por examen
    promedios = {}
    for examen in ['A', 'B', 'C']:
    ...
    # Bonus por diversidad (como estaba antes)
    bonus_diversidad = 0
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        notas_examen = [notas[i] for i in indices]
    ...
    # NUEVA RESTRICCIÓN: penalizar si un grupo tiene todos los alumnos con nota < 11
    penalizacion = 0
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        if all(notas[i] < 11 for i in indices):
            penalizacion += 1  # Penaliza con +1 por cada grupo que viole la regla

    # Fitness final
    fitness = -desv_promedios + bonus_diversidad - penalizacion * 2  # penalización fuerte
    return fitness
